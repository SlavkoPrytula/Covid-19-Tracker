...
class Prediction:
    ...
    def prediction(dates, values, x):
        ...
        for item in x:
            ...
            predictions.append(svr_rbf.predict(item)[0])
        # plt.show()
...
if __name__ == '__main__':
    ...
    number_days = p.get_days(["2020", "07", "20"])
    print(number_days)
    # for i in range()
    item = [i for i in range(len(data[0]))]
    # a = p.prediction(data[0], data[1], number_days[1])
    a = p.prediction(item, data[1], number_days[1])

    print(a, "here")
    print("ok")
    p.plot_graph(data[2], data[1], number_days[0], a)
